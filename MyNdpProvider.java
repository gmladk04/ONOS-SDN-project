...
public class MyNdpProvider extends AbstractProvider {
        ...
        public void process(PacketContext context) {
            ...
            if(eth.getEtherType()==(short)0x86dd && ipv6.getNextHeader()==17 && udp.getDestinationPort()==12345) {
                ...
                if(ns.getCode()==0){ //Registration
                    ...
                    installFlowRule(DeviceId.deviceId("of:0000000000000003"), rsuIpDB.get(DeviceId.deviceId("of:1000000000000001")), true);
                    // set RSU 's flow rule
                    installFlowRule(DeviceId.deviceId("of:0000000000000001"), rsuIpDB.get(DeviceId.deviceId("of:1000000000000001")), false);
                    ...
                    if(!addrDB.containsKey(target_addr)){ // no entry - send error NDP-NA
                        sendErrorNa2(target_addr, vehicle_mac);
                    }
                    // modify LBR 's flow rule
                    modifyFlowRule(DeviceId.deviceId("of:0000000000000003"), lbrFlowDB.get(1), rsuIpDB.get(DeviceId.deviceId("of:1000000000000002")), true);
                    modifyFlowRule(DeviceId.deviceId("of:0000000000000001"), lbrFlowDB.get(1), rsuIpDB.get(DeviceId.deviceId("of:1000000000000002")), true);
                    // modify RSU's flow rule
                    modifyFlowRule(DeviceId.deviceId("of:0000000000000003"), rsuFlowDB.get(1), IpAddress.valueOf(IpAddress.Version.INET6,target_addr), false);
                    modifyFlowRule(DeviceId.deviceId("of:0000000000000001"), rsuFlowDB.get(1), IpAddress.valueOf(IpAddress.Version.INET6,target_addr), false);
                    sendSuccessNa(target_addr, vehicle_mac);
                    ...
                    if(addrDB2.containsKey(target_addr)){ //entry exist - send error UDP-NA
                        sendErrorNa3(target_addr, vehicle_mac);
                    }
                    else{ // no entry - normal situation, insert new entry with the target address
                    ...
                    installFlowRule2(DeviceId.deviceId("of:0000000000000004"), rsuIpDB2.get(DeviceId.deviceId("of:1000000000000003")), true);
                    // set RSU 's flow rule
                    installFlowRule2(DeviceId.deviceId("of:0000000000000002"), rsuIpDB2.get(DeviceId.deviceId("of:1000000000000003")), false);
                    //IpAddress.valueOf(IpAddress.Version.INET6,target_addr)
                    sendSuccessNa3(target_addr, vehicle_mac);

                    modifyFlowRule2(DeviceId.deviceId("of:0000000000000004"), lbrFlowDB.get(1), rsuIpDB2.get(DeviceId.deviceId("of:1000000000000003")), true);
                    // modify RSU's flow rule
                    modifyFlowRule2(DeviceId.deviceId("of:0000000000000002"), rsuFlowDB.get(1), IpAddress.valueOf(IpAddress.Version.INET6,target_addr), false);
                    sendSuccessNa2(target_addr, vehicle_mac);
    ...
    private void modifyFlowRule2(DeviceId deviceId, FlowRule preRule, IpAddress ipv6, boolean isLBR){
        ...
        if(isLBR) {
            ...
            lbrFlowDB2.put(2, flowRule.build()); log.info("remove entry to DB - done -lbr");
            ...
            rsuFlowDB2.put(2, flowRule.build()); log.info("remove entry to DB - done -Rsu");
    ...
    private void sendSuccessNa(byte[] target_addr, MacAddress vehicle_mac) {
        ...
        OutboundPacket pkt = new DefaultOutboundPacket(DeviceId.deviceId("of:1000000000000001"), DefaultTrafficTreatment.builder().setOutput(PortNumber.portNumber(2)).build(), ByteBuffer.wrap(ethPacket.serialize()));
    ...
    private void sendSuccessNa2(byte[] target_addr, MacAddress vehicle_mac) {
        ...
        udpPacket.setSourcePort(1234);
        //ip contents
        byte[] sourceaddr = new byte[] {(byte)0x12, 0x12, 0x35, 0x35, 0x12, 0x12, 0x34, 0x34,0x12, 0x12, 0x34, 0x34,0x12, 0x12, 0x34, 0x34};
        byte[] destaddr = new byte[] {(byte)0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x0, 0x0, 0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1 };
        ipPacket.setPayload(udpPacket);
        ipPacket.setDestinationAddress(sourceaddr);
        ipPacket.setSourceAddress(destaddr);
        ...
        OutboundPacket pkt = new DefaultOutboundPacket(DeviceId.deviceId("of:1000000000000002"), DefaultTrafficTreatment.builder().setOutput(PortNumber.portNumber(2)).build(), ByteBuffer.wrap(ethPacket.serialize()));
        packetService.emit(pkt);
    }
    private void sendSuccessNa3(byte[] target_addr, MacAddress vehicle_mac) {
        ...
        udpPacket.setSourcePort(1234);
        //ip contents
        byte[] sourceaddr = new byte[] {(byte)0x12, 0x12, 0x12, 0x15, 0x12, 0x12, 0x34, 0x34,0x12, 0x12, 0x34, 0x34,0x12, 0x12, 0x34, 0x34};
        byte[] destaddr = new byte[] {(byte)  0x12, 0x34, 0x12, 0x34, 0x12, 0x34, 0x0, 0x0, 0x1,0x1,0x1,0x1,0x1,0x1,0x1,0x1};
        ...
        OutboundPacket pkt = new DefaultOutboundPacket(DeviceId.deviceId("of:1000000000000003"), DefaultTrafficTreatment.builder().setOutput(PortNumber.portNumber(2)).build(), ByteBuffer.wrap(ethPacket.serialize()));
        packetService.emit(pkt);
    }
    ...
    private void sendErrorNa2(byte[] target_addr, MacAddress vehicle_mac) {
        log.info("send Error NA");
        Ethernet ethPacket = new Ethernet();
        IPv6 ipPacket = new IPv6();
        UDP udpPacket = new UDP();
    ...
    private void sendErrorNa3(byte[] target_addr, MacAddress vehicle_mac) {
        log.info("send Error NA");
        Ethernet ethPacket = new Ethernet();
        IPv6 ipPacket = new IPv6();
        UDP udpPacket = new UDP();
