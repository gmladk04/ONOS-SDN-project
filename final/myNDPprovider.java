...
public class MyNdpProvider extends AbstractProvider {
   ...
   // ConcurrentHashMap<DeviceId, IpAddress> rsuIpDB2= new ConcurrentHashMap<DeviceId, IpAddress>();
    ...
    ConcurrentHashMap<DeviceId, String> rsuLbr1DB = new ConcurrentHashMap<DeviceId, String>();
    ConcurrentHashMap<DeviceId, String> rsuLbr2DB = new ConcurrentHashMap<DeviceId, String>();
    /**
    ...
    public void activate(ComponentContext context) {
        ...
        //add RSU ip addr
        rsuLbr1DB.put(DeviceId.deviceId("of:1000000000000001"), "lbr1");
        rsuNameDB.put(DeviceId.deviceId("of:1000000000000001"), "rsu1");

        rsuLbr1DB.put(DeviceId.deviceId("of:1000000000000002"), "lbr1");
        rsuNameDB.put(DeviceId.deviceId("of:1000000000000002"), "rsu2");

        rsuLbr2DB.put(DeviceId.deviceId("of:1000000000000003"), "lbr2");
    ...
    private void requestIntercepts() {
        ...
        selector.matchUdpDst(TpPort.tpPort(54321));
     ...
     */
    private void updateMac(DeviceId target, MacAddress rsuMac, int port) {
    ...
    private class InternalPacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
          ...
          ///  log.info("Next Header: " + ipv6.getNextHeader());
            ...
            if(udp.getDestinationPort() != 54321) {
                return;
            }

            if(eth.getEtherType()==(short)0x86dd && ipv6.getNextHeader()==17 && udp.getDestinationPort()==54321) {
                ...
                if(deviceId.toString().equals("of:1000000000000001")) {
                    updateMac(DeviceId.deviceId("of:0000000000000001"), macforrsu, 1);
                    updateMac(DeviceId.deviceId("of:0000000000000002"), macforrsu, 3);
                    updateMac(DeviceId.deviceId("of:0000000000000003"), macforrsu, 1);
                    updateMac(DeviceId.deviceId("of:0000000000000004"), macforrsu, 3);
                    updateMac(DeviceId.deviceId("of:0000000000000005"), macforrsu, 3);
                } else if (deviceId.toString().equals("of:1000000000000002")){
                    updateMac(DeviceId.deviceId("of:0000000000000001"), macforrsu, 2);
                    updateMac(DeviceId.deviceId("of:0000000000000002"), macforrsu, 3);
                    updateMac(DeviceId.deviceId("of:0000000000000003"), macforrsu, 1);
                    updateMac(DeviceId.deviceId("of:0000000000000004"), macforrsu, 3);
                    updateMac(DeviceId.deviceId("of:0000000000000005"), macforrsu, 3);
                } else if(deviceId.toString().equals("of:1000000000000003")) {
                    updateMac(DeviceId.deviceId("of:0000000000000001"), macforrsu, 4);
                    updateMac(DeviceId.deviceId("of:0000000000000002"), macforrsu, 1);
                    updateMac(DeviceId.deviceId("of:0000000000000003"), macforrsu, 2);
                    updateMac(DeviceId.deviceId("of:0000000000000004"), macforrsu, 1);
                    updateMac(DeviceId.deviceId("of:0000000000000005"), macforrsu, 2);
                }
                ...
                if(ns.getCode()==0){ //Registration
                    ...
                    if(rsuLbr1DB.get(deviceId) != null) {
                      //  log.info(rsuIpDB.get(deviceId).toString()); // print IPv6 address of the RSU which received the NS
                       // installFlowRuleToLbr(DeviceId.deviceId("of:0000000000000003"), targetAddr, rsuIpDB.get(deviceId),vehicle_mac);
                        installFlowRuleToLbr(DeviceId.deviceId("of:0000000000000003"), targetAddr, vehicle_mac);// Change the IPv6 address using deviceId (The switch which received this NS)
                        log.info("1");
                        installFlowRuleToRsu(deviceId, targetAddr, vehicle_mac);
                        log.info("1");
                    } else if (rsuLbr2DB.get(deviceId) != null) {
                       // log.info(rsuIpDB2.get(deviceId).toString()); // print IPv6 address of the RSU which received the NS
                        //installFlowRuleToLbr2(DeviceId.deviceId("of:0000000000000004"), targetAddr, rsuIpDB2.get(deviceId),vehicle_mac);
                        installFlowRuleToLbr2(DeviceId.deviceId("of:0000000000000004"), targetAddr, vehicle_mac);// Change the IPv6 address using deviceId (The switch which received this NS)
                        log.info("2");
                        installFlowRuleToRsu2(deviceId, targetAddr,vehicle_mac);
                        log.info("2");
                    }

                    sendSuccessNa(deviceId, target_addr, vehicle_mac, ipv6.getDestinationAddress(), eth.getDestinationMAC(), ipv6.getSourceAddress());
                }
                   ...
                   // modifyFlowRuleOfLbr(DeviceId.deviceId("of:0000000000000003"), targetAddr, rsuIpDB.get(deviceId),vehicle_mac);
                    modifyFlowRuleOfLbr(DeviceId.deviceId("of:0000000000000003"), targetAddr, vehicle_mac);// Change to the RSU which received the NS
                    // modify RSU's flow rule
                    modifyFlowRuleOfRsu(deviceId, targetAddr,vehicle_mac); // Change to deviceId and targetAddr

                    sendSuccessNa(deviceId, target_addr, vehicle_mac, ipv6.getDestinationAddress(), eth.getDestinationMAC(), ipv6.getSourceAddress()); // Return NA to the RSU which received the NS
                }
                    ...
                    installFlowRuleToRsu2(deviceId, targetAddr,vehicle_mac);
                    // modify LBR 's flow rule
                    // modifyFlowRuleOfLbr2(DeviceId.deviceId("of:0000000000000003"), targetAddr, rsuIpDB.get(DeviceId.deviceId("of:1000000000000002")),vehicle_mac);
                    modifyFlowRuleOfLbr(DeviceId.deviceId("of:0000000000000003"), targetAddr, vehicle_mac);// Change to the RSU which received the NS
                    // modify RSU's flow rule
                    modifyFlowRuleOfRsu(deviceId, targetAddr,vehicle_mac); // Change to deviceId and targetAddr
    ...
    private void installFlowRuleToRsu(DeviceId deviceId, IpAddress target, MacAddress vehicle_mac) {
        ...
        TrafficSelector.Builder selector = DefaultTrafficSelector.builder();
        selector.matchEthType((short)0x86dd);
        selector.matchIPv6Dst(IpPrefix.valueOf(target, 128)); // RSU IP
        ...
        rsuFlowDB.put(target, newFlow); log.info("put entry to DB - doneip -6  -Rsu");
    }

    private void installFlowRuleToLbr(DeviceId deviceId, IpAddress target, MacAddress vehicle_mac){
      ...
      //  treatment.setIpv6Dst(target);
        ...
        if(rsuLbr1DB.get(addrDB.get(target).deviceId) == null) {
    ...
    private void installFlowRuleToRsu2(DeviceId deviceId, IpAddress target, MacAddress vehicle_mac) {
        ...
        selector.matchIPv6Dst(IpPrefix.valueOf(target, 128));
    ...
    private void installFlowRuleToLbr2(DeviceId deviceId, IpAddress target, MacAddress vehicle_mac){
        log.info("install new flow rule (registration) to LBR");
        log.info(deviceId.toString());
    ...
    private void modifyFlowRuleOfLbr(DeviceId deviceId, IpAddress target,  MacAddress vehicle_mac) {
        log.info("modify LBR");
        log.info(deviceId.toString());

        flowRuleService.removeFlowRules(lbrFlowDB.get(target)); log.info("rule remove done");

        installFlowRuleToLbr(deviceId, target, vehicle_mac);
    }
    ...
    private void modifyFlowRuleOfLbr2(DeviceId deviceId, IpAddress target,  MacAddress vehicle_mac) {
        log.info("modify LBR2");
        log.info(deviceId.toString());

        flowRuleService.removeFlowRules(lbrFlowDB.get(target)); log.info("rule remove done");

         installFlowRuleToLbr2(deviceId, target, vehicle_mac);
    }

    private void sendSuccessNa(DeviceId deviceId, byte[] target_addr, MacAddress vehicle_mac, byte[] gw_ip_addr, MacAddress gw_mac, byte[] source_ip) {
        ...
        udpPacket.setDestinationPort(54321);
        udpPacket.setSourcePort(54321);

        //ip contents
        byte[] sourceaddr = gw_ip_addr;
        byte[] destaddr;
        byte[] nullAddr = new byte[] {(byte)0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        if(java.util.Arrays.equals(source_ip, nullAddr) == true) {
            destaddr = new byte[]{(byte) 0xff, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01};
        } else{
            destaddr = source_ip;
        }
        ...
        ethPacket.setSourceMACAddress(gw_mac).setPayload(ipPacket);
        ethPacket.setDestinationMACAddress(vehicle_mac);
